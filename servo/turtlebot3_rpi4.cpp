...
void TurtleBot3::add_sensors()
{
  ...
  sensors_.push_back(
    new sensors::BatteryState(node_handle_, "battery_state"));
  ...
  sensors_.push_back(
    new sensors::JointState(node_handle_, dxl_sdk_wrapper_, "joint_states", "base_link"));
}
...
void TurtleBot3::run()
{
  ...
  cmd_vel_callback();
  servo_angle_callback();  // 서보 제어 토픽 구독 등록
}
...
void TurtleBot3::servo_angle_callback()
{
  servo_angle_sub_ = node_handle_->create_subscription<std_msgs::msg::Int32>(
    "servo_angle",
    10,
    [this](const std_msgs::msg::Int32::SharedPtr msg) {
      int32_t angle = std::clamp((int32_t)msg->data, (int32_t)0, (int32_t)180);
      std::string err_msg;
      dxl_sdk_wrapper_->set_data_to_device(
        350,
        sizeof(angle),
        reinterpret_cast<uint8_t *>(&angle),
        &err_msg);
    }
  );
}
